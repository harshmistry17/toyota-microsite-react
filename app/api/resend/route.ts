...
export async function POST(req: Request) {
  try {
    ...
    console.log("Received resend request:", { uid, name, email, city })
  ...
  auth: {
    ...
    if (normalizedCity === "chennai") {
      emailBody = `
        <div style="font-family: Arial, sans-serif; padding: 20px; text-align: left; color: #333;">
          ...
          <p style="font-size: 16px; line-height: 1.5;">
            We hope you have received your buddy pass for the Toyota Drum Tao Concert.<br><br>
            In case you didn't, we are resharing it in this email for your convenience.
          </p>
          <p style="font-size: 16px; line-height: 1.5;">
      ...
      emailBody = `
        <div style="font-family: Arial, sans-serif; padding: 20px; text-align: left; color: #333;">
          <h1 style="color: #000;">Hi ${name},</h1>
          <p style="font-size: 16px; line-height: 1.5;">
            We hope you have received your registration confirmation for the Toyota Drum Tao Concert.<br><br>
            In case you didn't, we are resharing it in this email for your convenience.
          </p>
          <p style="font-size: 16px; line-height: 1.5;">
    ...
    const [emailResult, dbResult] = await Promise.all([
      ...
      supabaseAdmin
        ...
        .update({ image_link: publicImageUrl, email_status: true, resend_status: true })
        .eq("uid", uid),
    ])
    ...
    return NextResponse.json({ success: true, message: "Ticket regenerated and email resent!" })
  } catch (error) {
    console.error("Full error in /api/resend:", error)
    return NextResponse.json(
