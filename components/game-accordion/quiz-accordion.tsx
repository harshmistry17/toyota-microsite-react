...
export default function QuizAccordion({ onComplete, completed, disabled = false }: QuizAccordionProps) {
  const [selectedAnswers, setSelectedAnswers] = useState<Record<number, string>>({})
  ...
  useEffect(() => {
    ...
    if (allAnswered && !completed) {
      ...
      // Small delay before calling onComplete to show final answer
      setTimeout(() => {
        onComplete(results)
      }, 500)
    }
  ...
  return (
    <GameAccordion title="Drum Tao Quiz" completed={completed} defaultExpanded={true} disabled={disabled}>
      <div className="space-y-6">
        {quizQuestions.map((question, index) => (
          <div key={question.id} className="mt-2">
                    ...
                    className={`w-full p-2 border-2 text-left font-medium transition-all ${
                      isSelected && isCorrect
                        ...
                        : isSelected && !isCorrect
                          ...
                          : isSelected
                            ? "border-red-500 bg-red-500/10"
                            : "border-gray-700 hover:border-gray-600 text-white cursor-pointer disabled:opacity-75"
                    } ${isAnswered && !isSelected ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    <span className="flex items-center justify-between">
                      <span>
